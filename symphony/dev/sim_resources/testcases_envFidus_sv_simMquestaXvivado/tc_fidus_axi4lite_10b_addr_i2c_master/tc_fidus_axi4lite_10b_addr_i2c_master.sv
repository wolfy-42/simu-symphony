...
module test_case();
	...
	sim_management s;
	parameter TC_NAME = "tc_fidus_10b_addr_i2c_master_bfm";
	
	// i2c device address
	`define I2C_ADDRESS 10'h33c	
   ...
   logic           	slave_scl_t;
	...
	logic [9:0]			iic_addr = `I2C_ADDRESS;
	logic	[3:0]			error_cnt;
	logic 				error_sent;
	logic					error_rcved;
   ...
   assign axi_bfm_channel.aresetn = tb.reset_if.rn;
	...
	fidus_i2c_master_bfm #(.ADDR_SIZE(10)) i2c_master_10b_addr_bfm;
   fidus_axi4lite_mst_bfm #(.AWIDTH(9), .DWIDTH(32), .OUTPUT_DRV_EDGE("rise"), .OUTPUT_DRV_DLY(0)) axi4lite;
	 
	// I2C master interface	
	fidus_i2c_master_if i2c_master_addr_10_if
	(
		.clk		(tb.clk_i2c_if.c),
		.reset	(tb.reset_if.r)
	);
	...
	// AXI i2c slave 
	axi_iic_10b_addr_slave  axi_iic_10b_addr_slave
	(
		.iic2intc_irpt    (),       
	...
	assign slave_sda_io = (i2c_master_addr_10_if.sda_t == 1'b0) ? i2c_master_addr_10_if.sda_out : 1'bz;
	assign i2c_master_addr_10_if.sda_in = slave_sda_io;
	pullup(slave_sda_io);
	
	assign slave_scl_io = (i2c_master_addr_10_if.scl_t == 1'b0) ? i2c_master_addr_10_if.scl_out : 1'bz;
	assign i2c_master_addr_10_if.scl_in = slave_scl_io;	
	pullup(slave_scl_io);	
	...
	initial begin
		...
		s.printMessage (TC_NAME, "Set clock 48MHz");
		...
		i2c_master_10b_addr_bfm = new(TC_NAME, i2c_master_addr_10_if, 1);
      ...
      // Initialize AXI BFM setings.
		...
		axi4lite.tWriteAXI4Lite(9'h110, {iic_addr[6:0],1'b0});	// slave address 7 LSB	
		axi4lite.tWriteAXI4Lite(9'h11C, iic_addr[9:7]);	// slave address 3 MSB
		...
		i2c_master_10b_addr_bfm.tSendMessage(`I2C_ADDRESS, 1, register_addr, 1, total_data_write, repeated_start, error_sent);
		#500;
		...
		axi4lite.tReadAXI4Lite(9'h20, axi_iic_slave_resp); 
		axi4lite.tReadAXI4Lite(9'h104, axi_iic_slave_resp);      // Status Write transaction: address, data, strobe.
		
		fork 		
			begin
				i2c_master_10b_addr_bfm.tReceiveMessage(`I2C_ADDRESS, 1, register_addr, 1, total_data_read, repeated_start, error_rcved);		
			end
			
			begin
				@(posedge i2c_master_addr_10_if.sda_t);
				@(posedge i2c_master_addr_10_if.sda_t);
				while (axi_iic_slave_resp != 32'hC8) begin
		...
		i2c_master_10b_addr_bfm.tSendMessage(`I2C_ADDRESS, 4, register_addr, 1, total_data_write, repeated_start, error_sent);
		...
		i2c_master_10b_addr_bfm.tReceiveMessage(`I2C_ADDRESS, 4, register_addr, 1, total_data_read, repeated_start, error_rcved);
		#500;
