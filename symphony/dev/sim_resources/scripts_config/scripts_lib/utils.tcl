...
proc simu_vlog {args} {
    puts_debug2 "==============utils::simu_vlog $args =================\n"
    ...
    while {$index < $arglen} {
        ...
        switch -glob $arg {
            ...
            -incr {}
            -timescale {
                set flags(timescale) [lindex $args [incr index]]
            }
            -work {
                set flags(work) [lindex $args [incr index]]
            }
            -L - -lib {
                lappend libs [lindex $args [incr index]]
            }
            -i - -incdir {
                lappend incdirs [lindex $args [incr index]]
            }
            -d - -define {
                lappend defines [lindex $args [incr index]]
            }
            -cover {
                set flags(cover) true
            }
	    -f  {
                set flags(filelist) [lindex $args [incr index]]
            }
            default {
                if {[string match {[-+]*} $arg ]} {
    ...
    # foreach i [array name flags] {
    #     puts "flags($i): '$flags($i)'"
    # }

    switch $::DEFAULT_SIMULATOR {
        vsim - ahdl_gui - ahdl_sh  {
            set args ""
            foreach i [array name flags] {
                switch -exact $i {
                    sv { lappend args "-sv" }
                    cover { lappend args $::COVERAGE_PARAMS }
                    work { lappend args "-work" "$flags($i)" }
                    timescale { lappend args "-timescale" "$flags($i)" }
		    filelist { lappend args "-f" "$flags($i)" }
                }
            }
            foreach lib $libs {
                lappend args "-L" $lib
            ...
            foreach incdir $incdirs {
                lappend args "+incdir+$incdir"
            }
            foreach define $defines {
                lappend args "+define+$define"
            }
            foreach src $srcs {
                lappend args $src
            }
        ...
        xsim {
            ...
            foreach i [array name flags] {
                switch -exact $i {
                    ...
                    work { lappend args "-work" "$flags($i)" }
                    timescale {
                        puts "simu_vlog: Warning: timescale not supported for XSIM complilation"
                    }
                }
            }
            foreach lib $libs {
            ...
            foreach incdir $incdirs {
                lappend args "-i" $incdir
            }
            foreach define $defines {
                lappend args "-d" $define
            }
            foreach src $srcs {
                lappend args $src
            }
            append args "$::XSIM_INITFILE "

            puts "xvlog $args"
            exec >&@stdout xvlog {*}$args
        }
...
proc simu_vcom {args} {
    puts_debug2 "==============utils::simu_vcom $args===================\n"
