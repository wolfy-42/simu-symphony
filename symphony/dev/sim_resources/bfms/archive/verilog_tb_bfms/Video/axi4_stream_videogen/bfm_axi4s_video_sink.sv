...
package bfm_axi4s_video_sink_pkg;

// PACKAGES
import sim_management_pkg::*;
import video_bfm_pkg::*;

class bfm_axi4s_video_sink #(
        COMP_WIDTH = 8,
        PIX_PER_CLK = 4,
    ...
    virtual axi4s_if #(.TDATA_WIDTH(TDATA_WIDTH)) io;   // AXI4S interface.

    event                       frame_complete; // Event triggered after full frame has been received
    video_frame #(COMP_WIDTH)   frame;          // Stores video frame that gets returned from BFM
    ...
    //-----------------------------------------------------------
    function video_frame#(COMP_WIDTH) fGetLastFrame ();
        return frame;
...
endclass : bfm_axi4s_video_sink
endpackage : bfm_axi4s_video_sink_pkg
