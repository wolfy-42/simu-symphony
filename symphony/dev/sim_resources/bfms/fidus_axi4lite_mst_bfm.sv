...
class fidus_axi4lite_mst_bfm # (
   ...
   task tWriteAXI4Lite(
      ...
      tWriteAXI4LiteWithStatus(wr_addr, wr_data, dummy_wr_resp_status, wr_data_str);
   endtask
   ...
   task tWriteAXI4LiteWithStatus(
      ...
      if (awready_timeout || wready_timeout || bvalid_timeout)
         ...
         if (messaging_status) begin
            if (wr_data_str == {(DWIDTH/8){1'b1}}) begin
                s.printMessage(CLASS_NAME,$sformatf("AXI WRITE    ADDR =   %X     DATA = %X", wr_addr, wr_data));
            end else begin
                s.printMessage(CLASS_NAME,$sformatf("AXI WRITE    ADDR =   %X     DATA = %X     WSTRB = %b", wr_addr, wr_data, wr_data_str));
            end
         end
      end
