...
package fidus_axis_video_source_bfm_pkg;

// PACKAGES
import sim_management_pkg::*;
import fidus_axis_video_frame_bfm_pkg::*;

class fidus_axis_video_source_bfm #(
    ...
    virtual fidus_axis_video_if #(.TDATA_WIDTH(TDATA_WIDTH)) io;   // AXI4-Stream interface
    ...
    function new (
        ...
        string          name = "fidus_axis_video_source_bfm"
    );
        int i;
    ...
    //      hblank: results in transmission gap at the end of each line; specify the number of clock
    //                  cycles
    //      vblank: results in transmission gap at the end of each frame; specify the number of
    //                  clock cycles, not the number of lines
    ...
    task tCreateandSendFrame(
        ...
        fidus_axis_video_frame_bfm #(.COMP_WIDTH(COMP_WIDTH)) tmp;
        if (type_colour_bar)
    ...
    //-----------------------------------------------------------
    task tSendFrame(
        input fidus_axis_video_frame_bfm#(COMP_WIDTH) frame,
        input int hblank = 0,
        input int vblank = 0,
        input int timeout = -1
    );
...
endclass : fidus_axis_video_source_bfm
endpackage : fidus_axis_video_source_bfm_pkg
