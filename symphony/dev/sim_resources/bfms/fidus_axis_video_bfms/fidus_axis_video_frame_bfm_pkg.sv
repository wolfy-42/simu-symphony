...
package fidus_axis_video_frame_bfm_pkg;
...
//----------------------------------------------
class fidus_axis_video_frame_bfm #(
    ...
    function new(
        ...
        if (name == "")
            this.CLASS_NAME = $sformatf("fidus_axis_video_frame_bfm#%0d",frame_count);
        else
    ...
    //-----------------------------------------------------------
    function bit fCheckFrame(fidus_axis_video_frame_bfm #(COMP_WIDTH) expected);
    ...
    function void fWriteRawFrame(string filename);
        ...
        f = $fopen(filename, "wb");
        assert (f!=0) else $fatal(2,$sformatf("Unable to open output image file %s", filename));
    ...
    //-----------------------------------------------------------
    // Instantiates new fidus_axis_video_frame_bfm with pixel components that are incremented consecutively
    ...
    //-----------------------------------------------------------
    static function fidus_axis_video_frame_bfm#(COMP_WIDTH) fMakeCounterFrame (
        int width,
        int height,
        eVideoFormat video_format
    );
        logic [COMP_WIDTH-1:0] comp_cntr [NUM_COMPONENTS];
        automatic fidus_axis_video_frame_bfm#(COMP_WIDTH) tmp = new(width, height, video_format);
        int row, col, comp;
    ...
    //-----------------------------------------------------------
    // Instantiates new fidus_axis_video_frame_bfm with colour bar pattern
    //      width: test frame width
    //      height: test frame height
    //      video_format: colourspace and subsampling
    //
    ...
    //-----------------------------------------------------------
    static function fidus_axis_video_frame_bfm#(COMP_WIDTH) fMakeColourBar (
        int width,
        int height, 
        eVideoFormat video_format
    );
...
endclass : fidus_axis_video_frame_bfm

endpackage : fidus_axis_video_frame_bfm_pkg
